...
package com.shinemo.publish.debug.jdivisitor.event.visitor;
...
public abstract class EmptyEventVisitor implements EventVisitor {
	...
	protected String getSid(Event event){
		return (String)event.request().getProperty("sid");
	}
	
	
	protected String getClassName(Event event){
		return (String)event.request().getProperty("className");
	}
	
	protected int getLine(Event event){
		return (Integer)event.request().getProperty("line");
	}
