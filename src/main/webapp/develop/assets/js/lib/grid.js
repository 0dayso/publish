...
;void (function ($) {
            ...
            dialogOpts: [{
                ...
                buttons: [{
                    ...
                    handle: function ($noty, event) {
                        ...
                        me.submit($.type(me.gridIns.proxy.update) === 'string'
                                        ? { url: me.gridIns.proxy.update, type: 'post' }
                                        : me.gridIns.proxy.update,
                                Util.parseParam(form.serialize()),
                                function ( json ) {
                                    if ( json.success ) {
                                        me.close();
    ...
    var pDialog = {
        ...
        packOpts: function () {
            ...
            return {
                custom: me.custom || false,
                style: {
                    width: +me.style.width ? me.style.width + 'px' : me.style.width,
                    height: +me.style.height ? me.style.height + 'px' : me.style.height,
                },
                type: me.style.type,
                ...
                header: me.header,
                text: me.text,
                template: typeof me.content === 'string'
                            ? /^(\.|#)[-_A-Za-z][-_A-Za-z0-9]+/.test(me.content)
                                ? $(me.content).html() : me.content
                            : me.content instanceof jQuery
                                ? me.content.html() : me.content.innerHTML,
                timeout: me.timeout,
                callback: {
                    onShow: function () {
